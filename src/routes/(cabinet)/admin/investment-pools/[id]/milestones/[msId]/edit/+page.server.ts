...
export const load: PageServerLoad = async ({ params }) => {
  ...
  const milestone = await db.milestone.findUnique({ where: { id: params.msId } });
  if (!milestone || milestone.poolId !== params.id) throw error(404, 'Milestone not found');
...
export const actions: Actions = {
  ...
  delete: async ({ params }) => {
    ...
    await db.milestone.delete({ where: { id: params.msId } });
