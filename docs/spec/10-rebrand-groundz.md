...
> **Сделано (добор):** hero лендинга (`(marketing)/+page.svelte` — тег, заголовок, подзаголовок, CTA, 4 stat-метрики) и страница `/pricing` переведены через `t()`; цифры hero на `.num`.
> **Осталось:** остальные секции лендинга, листинг/детальная `/pools`, кабинет, формы. Прототип содержит обе локали — брать тексты оттуда.
