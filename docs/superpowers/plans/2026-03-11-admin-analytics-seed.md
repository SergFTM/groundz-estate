# Admin Analytics Dashboard + Seed Data Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Enrich the admin dashboard with conversion/revenue KPIs and breakdown panels, and expand seed data to 12 leads, 6 users, 10 payments, and 2 commissions for realistic local testing.

**Architecture:** Two independent tasks executed sequentially. Seed first (provides data), then dashboard extensions (consume data). Both are isolated file changes with no new dependencies.

**Tech Stack:** SvelteKit, Svelte 5 runes, Prisma + SQLite, bcryptjs, TypeScript

**Spec:** `docs/superpowers/specs/2026-03-11-admin-analytics-seed-design.md`

---

## Chunk 1: Seed Data Enrichment

**Files:**

- Modify: `prisma/seed.ts` (full replacement)

---

### Task 1: Replace seed.ts with enriched data

- [ ] **Step 1.1: Replace `prisma/seed.ts` with the full enriched seed**
...
async function main() {
  ...
  await prisma.payment.createMany({
    data: [
      // buyer (Maria Petrova) — SYM-202: 3 paid + 1 overdue + 3 upcoming
...
Seeding database...
  ...
  Created 10 payments (2 overdue, 5 paid, 3 upcoming)
  Created 5 documents
  Created 12 leads (new: 4, contacted: 3, converted: 3, lost: 2)
  Created 3 lead notes
  Created 2 commissions
  Created 2 investment pools + 2 investor investments
  Created 4 job positions
  Created 6 FAQ entries
  Created 3 articles

✓ Seeding complete!
```

If errors appear: check that `src/generated/prisma/client.js` exists. If not, run `npx prisma generate` first.

- [ ] **Step 1.3: Verify idempotency — run seed a second time**

```bash
npx prisma db seed
```

Expected: Same output with no errors. Confirms `deleteMany()` order is correct.

- [ ] **Step 1.4: Commit**

```bash
git add prisma/seed.ts
git commit -m "feat: expand seed data — 6 users, 12 leads, 10 payments, 2 commissions"
```

---

## Chunk 2: Admin Analytics Dashboard Extensions

**Files:**

- Modify: `src/routes/(cabinet)/admin/+page.server.ts` (add 7 queries)
- Modify: `src/routes/(cabinet)/admin/+page.svelte` (add 2 new sections)

---

### Task 2: Extend server-side data loading

- [ ] **Step 2.1: Replace `+page.server.ts` with extended version**
...
Expected: No errors. If `groupBy` types cause issues, confirm `_count` is accessed as a plain number (Prisma returns `number` directly from `groupBy` with `_count: true`).

---

### Task 3: Add new UI sections to the dashboard

- [ ] **Step 3.1: Add Sales & Revenue KPI row and breakdown panels to `+page.svelte`**

Open `src/routes/(cabinet)/admin/+page.svelte`.

**At the top of `<script>`, add the ProgressBar import** (after existing imports):

```svelte
import ProgressBar from '$lib/components/ui/ProgressBar.svelte';
```

**After the existing `admin-dash__kpis--secondary` div (line ~56), add:**
...
**In the `<style>` block, add these rules before the `@media` query:**
...
- [ ] **Step 3.3: Start dev server and visually verify**
...
3. You should see on `/admin`:
   ...
   - **Row 3 (new):** Conversion Rate = 25%, Units Sold = 8, Units Reserved = 3, Revenue Paid = €113,000
   - **Breakdown panels (new):** Leads by Status (4 bars), Units by Type (5 bars)
4. Confirm no console errors

- [ ] **Step 3.4: Commit**

```bash
git add src/routes/\(cabinet\)/admin/+page.server.ts src/routes/\(cabinet\)/admin/+page.svelte
git commit -m "feat: add conversion rate, sales KPIs, and breakdown panels to admin dashboard"
```
